<script>
    ...
    // Expanded list of chicken conspiracy theories
    const conspiracies = [
    ...
    let currentIndex = 0;
    let score = 0;
    let currentTheory = shuffledConspiracies[currentIndex];
    let resultMessage = '';
    let hintMessage = '';
  ...
  >
    <div class="container justify-center weird-container">
      ...
      <div class="game-buttons">
        <button on:click={() => checkGuess('real')} class="px-6 py-2 text-white rounded-full">Real</button>
        <button on:click={() => checkGuess('fake')} class="px-6 py-2 text-white rounded-full">Fake</button>
      </div>
      ...
      <div class="hint-section">
        <button on:click={generateHint} class="px-6 py-2 text-white rounded-full">Need a Hint?</button>
        {#if hintMessage}
